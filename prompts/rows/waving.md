Create a single horizontal sprite strip for the Codex app digital pet `frankie` in the state `waving`.
...
Output exactly 4 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: cute sausage dog dachshund pet, long low body, rounded snout, floppy ears, short legs, happy expressive face, warm tan and chestnut markings, no collar, no accessories.
